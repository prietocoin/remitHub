...
const { resolverUrlImagen } = require('../../../utils/image.util');

// Se usa la cola unificada central de remitHub
...
async function releerIA(req, res) {
  try {
    ...
    // Enviar a la cola central del pipeline
    ...
    return res.status(500).json({ false: true, error: err.message });
  }
}
