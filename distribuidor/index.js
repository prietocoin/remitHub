...
const { enviarWebhookFinal } = require('./src/services/webhook');
...
const worker = new Worker('cola-distribuidor', async (job) => {
  const { hash_largo, instancia } = job.data;
  ...
  try {
    // 1. Obtener los metadatos de los impactos (JIDs, Grupos, Captions y Pushnames)
    ...
    // 2. Obtener los datos extraídos por la IA y asentados
    ...
    // 3. Construir el Payload Maestro
    const payload = {
    ...
    // 4. Enviar Webhook
    await enviarWebhookFinal(payload);
...
worker.on('completed', (job) => console.log(`[Distribuidor Evento] 🎉 Job ${job.id} completado.`));
worker.on('failed', (job, err) => console.error(`[Distribuidor Evento] ❌ Job ${job?.id} falló:`, err.message));

console.log('[Distribuidor] 🟢 Worker activo y escuchando "cola-distribuidor"...');
